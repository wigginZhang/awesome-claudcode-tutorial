---
layout: home

hero:
  name: "Claude Code 教程"
  text: "🤖 AI编程助手完整学习指南"
  tagline: 从零基础到高级应用，212+篇精选文章助你精通 Claude Code | 国内可用，无需科学上网
  image:
    src: /logo.svg
    alt: Claude Code
  actions:
    - theme: brand
      text: 🎓 课程式学习（29章）
      link: /zh/chapters/
    - theme: alt
      text: 📖 文章式学习（212+篇）
      link: /zh/articles/
    - theme: alt
      text: 🌐 English
      link: /en/

features:
  - icon: 🎓
    title: 系统化学习路径
    details: 10大章节、212+篇精选文章，从基础概念到高级应用，循序渐进掌握 Claude Code 的每一个功能点
  - icon: 💡
    title: 核心功能精讲
    details: 深度解析代码生成、智能补全、代码重构、错误诊断等核心功能，配合实战案例即学即用
  - icon: 🔧
    title: 高级特性探索
    details: 掌握自定义配置、插件开发、工作流优化、团队协作等进阶技巧，释放 AI 编程的全部潜力
  - icon: 📊
    title: 实战项目案例
    details: 涵盖前端、后端、全栈、DevOps等多领域真实项目，手把手教你将 Claude Code 应用到实际开发
  - icon: 🎯
    title: 最佳实践指南
    details: 总结业界最佳实践，避免常见陷阱，提供性能优化、安全防护、代码质量保障等专业建议
  - icon: 🌐
    title: 完全开源免费
    details: 所有教程内容完全开源，持续更新，欢迎贡献和反馈，让我们共同打造最好的中文 Claude Code 教程
---

<div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 24px; margin: 50px 0;">
  <a href="/zh/chapters/" style="text-decoration: none;">
    <div style="background: #f6f8fa; padding: 30px; border-radius: 12px; border: 2px solid #159957; transition: transform 0.2s, box-shadow 0.2s;">
      <div style="font-size: 2.5em; margin-bottom: 15px;">🎓</div>
      <h3 style="margin-bottom: 12px; color: #159957;">课程式学习</h3>
      <p style="color: #555; margin-bottom: 15px;">29 个结构化章节，从入门到企业级应用，循序渐进系统掌握 Claude Code</p>
      <div style="display: flex; gap: 15px; flex-wrap: wrap;">
        <span style="background: rgba(21,153,87,0.1); color: #159957; padding: 4px 12px; border-radius: 20px; font-size: 0.85em;">29 章节</span>
        <span style="background: rgba(21,153,87,0.1); color: #159957; padding: 4px 12px; border-radius: 20px; font-size: 0.85em;">系统学习</span>
        <span style="background: rgba(21,153,87,0.1); color: #159957; padding: 4px 12px; border-radius: 20px; font-size: 0.85em;">实战导向</span>
      </div>
    </div>
  </a>
  <a href="/zh/articles/" style="text-decoration: none;">
    <div style="background: #f6f8fa; padding: 30px; border-radius: 12px; border: 2px solid #155799; transition: transform 0.2s, box-shadow 0.2s;">
      <div style="font-size: 2.5em; margin-bottom: 15px;">📖</div>
      <h3 style="margin-bottom: 12px; color: #155799;">文章式学习</h3>
      <p style="color: #555; margin-bottom: 15px;">212+ 篇详细文章，按主题分类，支持全文搜索，快速查阅特定知识点</p>
      <div style="display: flex; gap: 15px; flex-wrap: wrap;">
        <span style="background: rgba(21,89,153,0.1); color: #155799; padding: 4px 12px; border-radius: 20px; font-size: 0.85em;">212+ 篇</span>
        <span style="background: rgba(21,89,153,0.1); color: #155799; padding: 4px 12px; border-radius: 20px; font-size: 0.85em;">全文搜索</span>
        <span style="background: rgba(21,89,153,0.1); color: #155799; padding: 4px 12px; border-radius: 20px; font-size: 0.85em;">快速查阅</span>
      </div>
    </div>
  </a>
</div>


## 📊 教程概览

<div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(240px, 1fr)); gap: 20px; margin: 40px 0;">
  <div style="background: linear-gradient(135deg, #155799 0%, #159957 100%); padding: 30px; border-radius: 12px; text-align: center; color: white;">
    <div style="font-size: 3em; font-weight: bold; margin-bottom: 10px;">212+</div>
    <div style="font-size: 1.2em; opacity: 0.95;">精选文章</div>
  </div>
  <div style="background: linear-gradient(135deg, #159957 0%, #38f9d7 100%); padding: 30px; border-radius: 12px; text-align: center; color: white;">
    <div style="font-size: 3em; font-weight: bold; margin-bottom: 10px;">10</div>
    <div style="font-size: 1.2em; opacity: 0.95;">核心章节</div>
  </div>
  <div style="background: linear-gradient(135deg, #1e6bb8 0%, #159957 100%); padding: 30px; border-radius: 12px; text-align: center; color: white;">
    <div style="font-size: 3em; font-weight: bold; margin-bottom: 10px;">50+</div>
    <div style="font-size: 1.2em; opacity: 0.95;">实战案例</div>
  </div>
  <div style="background: linear-gradient(135deg, #155799 0%, #1e6bb8 100%); padding: 30px; border-radius: 12px; text-align: center; color: white;">
    <div style="font-size: 3em; font-weight: bold; margin-bottom: 10px;">100%</div>
    <div style="font-size: 1.2em; opacity: 0.95;">开源免费</div>
  </div>
</div>

## 🗺️ 学习路径导航

### 🌱 第一阶段：入门基础（第1-2章）

<div style="background: #f6f8fa; padding: 25px; border-radius: 10px; border-left: 4px solid #159957; margin: 20px 0;">

**第一章：Claude Code 入门指南**
- 🎯 核心内容：什么是 Claude Code、安装配置、基础概念、快速上手
- 📝 文章数量：20+ 篇
- ⏱️ 预计学习时间：3-5 天
- 🎓 学习目标：了解 Claude Code 的核心价值，完成环境搭建，掌握基本操作

**第二章：核心功能详解**
- 🎯 核心内容：代码生成、智能补全、代码重构、错误诊断、代码审查
- 📝 文章数量：30+ 篇
- ⏱️ 预计学习时间：5-7 天
- 🎓 学习目标：熟练使用 Claude Code 的核心功能，提升日常开发效率

</div>

### 🚀 第二阶段：进阶应用（第3-5章）

<div style="background: #f6f8fa; padding: 25px; border-radius: 10px; border-left: 4px solid #155799; margin: 20px 0;">

**第三章：高级特性探索**
- 🎯 核心内容：自定义配置、快捷键设置、工作流优化、性能调优
- 📝 文章数量：25+ 篇
- ⏱️ 预计学习时间：4-6 天
- 🎓 学习目标：深度定制 Claude Code，打造个性化开发环境

**第四章：实战项目应用**
- 🎯 核心内容：前端项目、后端API、全栈应用、DevOps实践
- 📝 文章数量：35+ 篇
- ⏱️ 预计学习时间：7-10 天
- 🎓 学习目标：在真实项目中应用 Claude Code，积累实战经验

**第五章：团队协作与最佳实践**
- 🎯 核心内容：代码规范、团队协作、代码质量保障、安全防护
- 📝 文章数量：20+ 篇
- ⏱️ 预计学习时间：3-5 天
- 🎓 学习目标：掌握团队协作技巧，建立最佳实践规范

</div>

### 🎯 第三阶段：专家进阶（第6-10章）

<div style="background: #f6f8fa; padding: 25px; border-radius: 10px; border-left: 4px solid #1e6bb8; margin: 20px 0;">

**第六章：插件开发与扩展**
- 🎯 核心内容：插件架构、API 使用、自定义插件开发、扩展机制
- 📝 文章数量：18+ 篇
- ⏱️ 预计学习时间：5-7 天
- 🎓 学习目标：开发自定义插件，扩展 Claude Code 功能

**第七章：AI 提示词工程**
- 🎯 核心内容：提示词设计、上下文管理、对话策略、效果优化
- 📝 文章数量：22+ 篇
- ⏱️ 预计学习时间：4-6 天
- 🎓 学习目标：掌握提示词工程技巧，提升 AI 输出质量

**第八章：性能优化与调试**
- 🎯 核心内容：性能分析、响应速度优化、资源管理、问题排查
- 📝 文章数量：15+ 篇
- ⏱️ 预计学习时间：3-5 天
- 🎓 学习目标：优化使用体验，解决常见问题

**第九章：企业级应用**
- 🎯 核心内容：私有化部署、权限管理、安全合规、大规模应用
- 📝 文章数量：12+ 篇
- ⏱️ 预计学习时间：3-4 天
- 🎓 学习目标：掌握企业级应用方案，满足商业场景需求

**第十章：未来展望与趋势**
- 🎯 核心内容：技术演进、行业趋势、未来方向、职业发展
- 📝 文章数量：10+ 篇
- ⏱️ 预计学习时间：2-3 天
- 🎓 学习目标：了解 AI 编程的未来发展，规划职业成长路径

</div>

## 💡 学习建议

<div style="background: linear-gradient(135deg, #f3f6fa 0%, #e8f5e9 100%); padding: 30px; border-radius: 12px; margin: 40px 0;">

### 📚 如何高效学习本教程？

1. **循序渐进**：建议按照章节顺序学习，先打好基础再进阶
2. **边学边练**：每学完一个知识点，立即在实际项目中尝试应用
3. **做好笔记**：记录学习心得和遇到的问题，建立个人知识库
4. **参与讨论**：加入社区讨论，与其他学习者交流经验
5. **持续实践**：AI 编程是实践性很强的技能，多用才能熟练

### 🎯 学习时间规划

- **快速入门**：1-2 周掌握基础功能，满足日常开发需求
- **熟练应用**：1-2 个月系统学习，达到熟练应用水平
- **专家级别**：3-6 个月深度学习，成为 Claude Code 专家

### 💪 学习成果检验

- ✅ 能够独立使用 Claude Code 完成日常开发任务
- ✅ 能够优化开发流程，提升至少 50% 的开发效率
- ✅ 能够处理复杂场景，解决实际项目中的问题
- ✅ 能够指导团队成员，建立最佳实践规范
- ✅ 能够开发自定义插件，扩展 Claude Code 功能

</div>

## 🤝 贡献指南

<div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 20px; margin: 40px 0;">
  <div style="background: #f6f8fa; padding: 25px; border-radius: 10px; border: 2px solid #159957;">
    <div style="font-size: 2em; margin-bottom: 15px;">🐛</div>
    <h3 style="margin-bottom: 15px;">发现错误？</h3>
    <p>如果你发现教程中的错误或不准确的内容，欢迎提交 Issue 告诉我们</p>
  </div>
  <div style="background: #f6f8fa; padding: 25px; border-radius: 10px; border: 2px solid #155799;">
    <div style="font-size: 2em; margin-bottom: 15px;">💡</div>
    <h3 style="margin-bottom: 15px;">有更好的建议？</h3>
    <p>如果你有更好的内容建议或改进方案，欢迎提交 Pull Request</p>
  </div>
  <div style="background: #f6f8fa; padding: 25px; border-radius: 10px; border: 2px solid #1e6bb8;">
    <div style="font-size: 2em; margin-bottom: 15px;">🌟</div>
    <h3 style="margin-bottom: 15px;">想贡献内容？</h3>
    <p>欢迎贡献新的文章、案例或翻译，让我们一起完善教程</p>
  </div>
</div>

### 参与方式

1. **Star ⭐** 本项目，关注最新更新
2. **Fork 🍴** 仓库，进行修改和改进
3. **提交 PR 📝**，分享你的贡献
4. **加入讨论 💬**，参与社区建设

访问 [GitHub 仓库](https://github.com/yourusername/claudecode-tutorial) 开始贡献

## 📄 许可协议

本教程基于 **MIT 许可协议** 开源，你可以自由：

- ✅ 使用：个人学习或商业使用
- ✅ 修改：根据需求修改内容
- ✅ 分发：分享给更多人
- ✅ 二次创作：基于本教程创作衍生作品

唯一的要求是保留原作者的版权声明。

## 🎉 开始学习

<div style="text-align: center; margin: 60px 0;">
  <div style="font-size: 4em; margin-bottom: 20px;">🚀</div>
  <h2 style="font-size: 2em; margin-bottom: 20px;">准备好开始你的 AI 编程之旅了吗？</h2>
  <p style="font-size: 1.2em; color: #666; margin-bottom: 30px;">29 个课程章节 + 212+ 篇精选文章，两种方式任你选择</p>
  <div style="display: flex; justify-content: center; gap: 20px; flex-wrap: wrap;">
    <a href="/zh/chapters/" style="display: inline-block; background: linear-gradient(120deg, #159957, #38f9d7); color: white; padding: 18px 50px; border-radius: 30px; font-size: 1.2em; font-weight: bold; text-decoration: none; box-shadow: 0 4px 20px rgba(21, 153, 87, 0.3);">
      🎓 课程式学习
    </a>
    <a href="/zh/articles/" style="display: inline-block; background: linear-gradient(120deg, #155799, #1e6bb8); color: white; padding: 18px 50px; border-radius: 30px; font-size: 1.2em; font-weight: bold; text-decoration: none; box-shadow: 0 4px 20px rgba(21, 89, 153, 0.3);">
      📖 文章式学习
    </a>
  </div>
</div>

---

<div style="text-align: center; padding: 40px 0; color: #999;">
  <p style="font-size: 1.1em;">用 ❤️ 打造 | 持续更新中 | 欢迎 Star ⭐</p>
  <p style="margin-top: 10px;">让 AI 编程助手成为你最得力的开发伙伴</p>
</div>
